...
# insert time before/after intervention T/F
...
```
```{r}
relevant_data<-subset(data,select=c(PeriodValue,block,intervention))
relevant_data_after<-relevant_data[!relevant_data$intervention=="Before intervention",]
relevant_data_after$intervention<-"After Intervention"
relevant_data<-rbind((relevant_data[relevant_data$intervention=="Before intervention",]),relevant_data_after)
dcast((aggregate(PeriodValue~.,data=relevant_data,FUN=mean)),block~intervention,value.var = "PeriodValue")
```


```{r}
relevant_data<-subset(data,select=c(PeriodValue,block,covid))
relevant_data_after<-relevant_data[relevant_data$covid==FALSE,]
relevant_data_after$covid<-"Not impacted by COVID"
relevant_data_before<-relevant_data[relevant_data$covid==TRUE,]
relevant_data_before$covid<-"Impacted by COVID"
relevant_data<-rbind(relevant_data_before,relevant_data_after)
dcast((aggregate(PeriodValue~.,data=relevant_data,FUN=mean)),block~covid,value.var = "PeriodValue")
```

```{r}
summary(data$PeriodValue)
```
